...
jQuery(document).ready(function () {
  ...
  jQuery('#wp-admin-bar-autoptimize-default li').on('click', function (e) {
    ...
    jQuery('#wp-admin-bar-autoptimize').removeClass('hover'); // Create and Show the Autoptimize Loading Modal
    ...
    var _success = function success() {
      ...
      jQuery('#wp-admin-bar-autoptimize-cache-info .autoptimize-radial-bar .fill').attr('class', 'fill bg-green'); // Reset the class names of bullet icon

      jQuery('#wp-admin-bar-autoptimize').attr('class', 'menupop bullet-green'); // Reset the Radial Bar progress
    ...
    jQuery.ajax({
      ...
      cache: false,
      timeout: 9000,
      success: function success(cleared) {
      ...
      error: function error(jqXHR, textStatus) {
        // Remove the Autoptimize Loading Modal
        modal_loading.remove(); // WordPress Admin Notice
