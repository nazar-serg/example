    ...
    get: function get(total, num) {
      ...
      $.post(ajaxurl, data, function (r) {
        ...
        if ('object' == _typeof(r) && r.responses[0]) {
          ...
          $('a[className*=\':\']').unbind(); // If the offset is over the total number of comments we cannot fetch any more, so hide the button.
...
jQuery(document).ready(function ($) {
  ...
  }).filter(':visible').find('.wp-tab-first').focus(); // Set the heartbeat interval to 15 seconds if post lock dialogs are enabled.
  ...
  $('#post-preview').on('click.post-preview', function (event) {
    ...
    $form.attr('target', target).submit().attr('target', ''); // Workaround for WebKit bug preventing a form submitting twice to the same action.
  ...
  $('.categorydiv').each(function () {
    ...
    if (getUserSetting(settingName)) $('a[href="#' + taxonomy + '-pop"]', '#' + taxonomy + '-tabs').click(); // Add category button controls.
