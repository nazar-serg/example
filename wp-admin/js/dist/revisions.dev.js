...
(function ($) {
  ...
  revisions.settings = window._wpRevisionsSettings || {}; // For debugging.
  ...
  revisions.model.Slider = Backbone.Model.extend({
    ...
    initialize: function initialize(options) {
      ...
      this.revisions = options.revisions; // Listen for changes to the revisions or mode from outside.

      this.listenTo(this.frame, 'update:revisions', this.receiveRevisions);
      this.listenTo(this.frame, 'change:compareTwoMode', this.updateMode); // Listen for internal changes.
      ...
      this.on('update:revisions', this.updateSliderSettings); // Listen for changes to the hovered revision.
    ...
    getSliderValue: function getSliderValue(a, b) {
    ...
    hoverRevision: function hoverRevision(model, value) {
      this.trigger('hovered:revision', value);
    },
    // Called when `compareTwoMode` changes.
    updateMode: function updateMode(model, value) {
      this.set({
        compareTwoMode: value
      });
    },
    ...
    receiveRevisions: function receiveRevisions(from, to) {
  ...
  revisions.model.Tooltip = Backbone.Model.extend({
    ...
    updateRevision: function updateRevision(revision) {
      this.set({
        revision: revision
      });
    },
    setHovering: function setHovering(model, value) {
      this.set({
        hovering: value
      });
    },
    setScrubbing: function setScrubbing(model, value) {
      this.set({
        scrubbing: value
      });
    }
  ...
  revisions.model.Diffs = Backbone.Collection.extend({
    ...
    ensure: function ensure(id, context) {
    ...
    getClosestUnloaded: function getClosestUnloaded(ids, centerId) {
      var self = this;
    ...
    _loadAll: function _loadAll(allRevisionIds, centerId, num) {
      var self = this,
          deferred = $.Deferred(),
    ...
    load: function load(comparisons) {
      wp.revisions.log('load', comparisons); // Our collection should only ever grow, never shrink, so `remove: false`.
    ...
    sync: function sync(method, model, options) {
      if ('read' === method) {
        ...
        var deferred = wp.ajax.send(options),
            requests = this.requests; // Record that we're requesting each diff.
        ...
        return deferred; // Otherwise, fall back to `Backbone.sync()`.
      } else {
  ...
  revisions.model.FrameState = Backbone.Model.extend({
    ...
    initialize: function initialize(attributes, options) {
      ...
      this.diffs.set(this.get('diffData')); // Set up internal listeners.
      ...
      this.listenTo(this, 'update:diff', this.updateLoadingStatus); // Set the initial revisions, baseUrl, and mode as provided through attributes.
      ...
      if (window.history && window.history.pushState) {
        this.router = new revisions.Router({
          model: this
        });
    ...
    changeMode: function changeMode(model, value) {
      var toIndex = this.revisions.indexOf(this.get('to')); // If we were on the first revision before switching to two-handled mode,
    ...
    updatedRevisions: function updatedRevisions(from, to) {
      if (this.get('compareTwoMode')) {// @todo Compare-two loading strategy.
      } else {
     ...
     */
    updateDiff: function updateDiff(options) {
      var from, to, diffId, diff;
      options = options || {};
      from = this.get('from');
      to = this.get('to');
      diffId = (from ? from.id : 0) + ':' + to.id; // Check if we're actually changing the diff id.
      ...
      this._diffId = diffId;
      this.trigger('update:revisions', from, to);
      diff = this.diffs.get(diffId); // If we already have the diff, then immediately trigger the update.

      if (diff) {
        this.receiveDiff(diff);
        return $.Deferred().resolve().promise(); // Otherwise, fetch the diff.
      } else {
    ...
    receiveDiff: function receiveDiff(diff) {
  ...
  revisions.view.Frame = wp.Backbone.View.extend({
    ...
    renderDiff: function renderDiff(diff) {
  ...
  revisions.view.Controls = wp.Backbone.View.extend({
    ...
    initialize: function initialize() {
      _.bindAll(this, 'setWidth'); // Add the button view.
    ...
    ready: function ready() {
      ...
      this.window = $(window);
      this.window.on('scroll.wp.revisions', {
        controls: this
      }, function (e) {
        ...
        if (scrolled >= controls.top) {
          if (!frame.$el.hasClass('pinned')) {
            ...
            controls.window.on('resize.wp.revisions.pinning click.wp.revisions.pinning', {
              controls: controls
            }, function (e) {
  ...
  revisions.view.Tickmarks = wp.Backbone.View.extend({
    ...
    reportTickPosition: function reportTickPosition(model, revision) {
      var offset,
          thisOffset,
          parentOffset,
          tick,
  ...
  revisions.view.Buttons = wp.Backbone.View.extend({
    ...
    gotoModel: function gotoModel(toIndex) {
      var attributes = {
    ...
    disabledButtonCheck: function disabledButtonCheck() {
      var maxVal = this.model.revisions.length - 1,
          ...
          val = this.model.revisions.indexOf(this.model.get('to')); // Disable "Next" button if you're on the last node.

      next.prop('disabled', maxVal === val); // Disable "Previous" button if you're on the first node.
  ...
  revisions.view.Slider = wp.Backbone.View.extend({
    ...
    start: function start(event, ui) {
      ...
      $(window).on('mousemove.wp.revisions', {
        view: this
      }, function (e) {
        var handles,
            view = e.data.view,
            ...
            leftDragReset = '0',
            rightDragReset = '100%',
            handle = $(ui.handle); // In two handle mode, ensure handles can't be dragged past each other.
    ...
    slide: function slide(event, ui) {
      var attributes, movedRevision; // Compare two revisions mode.
      ...
      movedRevision = this.model.revisions.at(this.getPosition(ui.value)); // If we are scrubbing, a scrub to a revision is considered a hover.
  ...
  revisions.Router = Backbone.Router.extend({
    initialize: function initialize(options) {
      this.model = options.model; // Maintain state and history when navigating.
    ...
    handleRoute: function handleRoute(a, b) {
  ...
  revisions.init = function () {
    var state; // Bail if the current page is not revision.php.
