...
(function ($, wp) {
  ...
  function configureLinting(editor, settings) {
    ...
    editor.setOption('lint', getLintOptions()); // Keep lint options populated.
    ...
    editor.on('blur', updateErrorNotice); // Work around hint selection with mouse causing focus to leave editor.
    ...
    editor.on('endCompletion', function () {
      ...
      editor.on('blur', updateErrorNotice); // Wait for editor to possibly get re-focused after selection.
  ...
  function configureTabbing(codemirror, settings) {
    ...
    codemirror.on('keydown', function onKeydown(editor, event) {
      var tabKeyCode = 9,
          escKeyCode = 27; // Take note of the ESC keypress so that the next TAB can focus outside the editor.
      ...
      $textarea.data('next-tab-blurs', false); // Prevent tab character from being added.
    ...
    if (codemirror.showHint) {
      codemirror.on('keyup', function (editor, event) {
        ...
        if ('html' === innerMode || 'xml' === innerMode) {
          shouldAutocomplete = '<' === event.key || '/' === event.key && 'tag' === token.type || isAlphaKey && 'tag' === token.type || isAlphaKey && 'attribute' === token.type || '=' === token.string && token.state.htmlState && token.state.htmlState.tagName;
