...
jQuery(document).ready(function (m) {
  ...
  o.click(function (e) {
    d.is(":hidden") && (d.slideDown("fast", function () {
      m("input, select", v).first().focus();
    }), m(this).hide()), e.preventDefault();
  }), d.find(".cancel-timestamp").click(function (e) {
    o.show().focus(), d.slideUp("fast"), m("#mm").val(m("#hidden_mm").val()), m("#jj").val(m("#hidden_jj").val()), m("#aa").val(m("#hidden_aa").val()), m("#hh").val(m("#hidden_hh").val()), m("#mn").val(m("#hidden_mn").val()), c.html(a), e.preventDefault();
