...
(function (window, $, undefined) {
  ...
  $(function () {
    var $wrap = $('#postdivrich'),
        ...
        mceEditor,
        mceBind = function mceBind() {},
        mceUnbind = function mceUnbind() {},
        fixedTop = false,
        fixedBottom = false,
        fixedSideTop = false,
    ...
    $document.on('tinymce-editor-init.editor-expand', function (event, editor) {
      ...
      hideFloatPanels = _.debounce(function () {
        ...
        $('.mce-tooltip').hide();
      }, 1000, true); // Make sure it's the main editor.
      ...
      mceEditor = editor; // Set the minimum height to the initial viewport height.

      editor.settings.autoresize_min_height = autoresizeMinHeight; // Get the necessary UI elements.
      ...
      function mceGetCursorOffset() {
        ...
        if (editor.wp && editor.wp.getView && (view = editor.wp.getView(node))) {
          ...
          range = editor.selection.getRng(); // Try to get the offset from a range.

          try {
            offset = range.getClientRects()[0];
          } catch (er) {} // Get the offset from the bounding client rectangle of the node.
      ...
      function mceKeyup(event) {
        var key = event.keyCode; // Bail on special keys. Key code 47 is a '/'.

        if (key <= 47 && !(key === VK.SPACEBAR || key === VK.ENTER || key === VK.DELETE || key === VK.BACKSPACE || key === VK.UP || key === VK.LEFT || key === VK.DOWN || key === VK.UP)) {
          return; // OS keys, function keys, num lock, scroll lock. Key code 91-93 are OS keys.
      ...
      function mceScroll(key) {
        var offset = mceGetCursorOffset(),
            ...
            cursorBottom,
            editorTop,
            editorBottom; // Don't scroll if there is no offset.
        ...
        cursorTop = offset.top + editor.iframeElement.getBoundingClientRect().top; // Determine the cursorBottom based on the cursorTop and offset height.

        cursorBottom = cursorTop + offset.height; // Subtract the buffer from the cursorTop.

        cursorTop = cursorTop - buffer; // Add the buffer to the cursorBottom.
        ...
        editorBottom = heights.windowHeight - (advanced ? heights.bottomHeight + heights.statusBarHeight : 0); // Don't scroll if the node is taller than the visible part of the editor.
      ...
      mceBind = function mceBind() {
        ...
        editor.on('wp-toolbar-toggle', toggleAdvanced); // Adjust when the editor resizes.

        editor.on('setcontent wp-autoresize wp-toolbar-toggle', adjust); // Don't hide the caret after undo/redo.

        editor.on('undo redo', mceScroll); // Adjust when exiting TinyMCE's fullscreen mode.
    ...
    function adjust(event) {
      ...
      if (!canPin) {
        ...
        if ((!fixedTop || resize) && windowPos >= topPos - heights.toolsHeight - heights.adminBarHeight && windowPos <= topPos - heights.toolsHeight - heights.adminBarHeight + editorHeight - buffer) {
          fixedTop = true;
          $tools.css({
            position: 'fixed',
        ...
        if ((!fixedBottom || resize && advanced) && // Add borderWidth for the border around the .wp-editor-container.
        ...
        } else if (!advanced && fixedBottom || (fixedBottom || resize) && windowPos + heights.windowHeight > editorPos + editorHeight + heights.bottomHeight + heights.statusBarHeight - borderWidth) {
          fixedBottom = false;
          $statusBar.attr('style', advanced ? '' : 'visibility: hidden;');
          $bottom.attr('style', '');
        }
      ...
      if ($postboxContainer.width() < 300 && heights.windowWidth > 600 && // Check if the sidebar is not taller than the document height.
      $document.height() > $sideSortables.height() + postBodyTop + 120 && // Check if the editor is taller than the viewport.
          ...
          if (windowPos + pinnedToolsTop <= postBodyTop) {
            ...
            if (windowPos > lastScrollPosition) {
              if (fixedSideTop) {
                ...
                footerTop = $footer.offset().top; // Don't get over the footer.
    ...
    function on() {
      ...
      $wrap.addClass('wp-editor-expand'); // Adjust when the window is scrolled or resized.
      ...
      $document.on('wp-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust).on('postbox-toggled.editor-expand postbox-moved.editor-expand', function () {
      ...
      mceBind(); // Adjust when entering or exiting fullscreen mode.
    ...
    function off() {
      ...
      mceUnbind(); // Adjust when entering or exiting fullscreen mode.

      fullscreen && fullscreen.pubsub.unsubscribe('hidden', fullscreenHide); // Reset all CSS.
    ...
    if ($wrap.hasClass('wp-editor-expand')) {
      on(); // Resize just after CSS has fully loaded and QuickTags is ready.
  ...
  $(function () {
    ...
    function fadeOut(event) {
      ...
      if (event && (event.metaKey || event.ctrlKey && !event.altKey || event.altKey && event.shiftKey || key && ( // Special keys ( tab, ctrl, alt, esc, arrow keys... ).
      key <= 47 && key !== 8 && key !== 13 && key !== 32 && key !== 46 || // Windows keys.
      key >= 91 && key <= 93 || // F keys.
      key >= 112 && key <= 135 || // Num Lock, Scroll Lock, OEM.
      key >= 144 && key <= 150 || // OEM or non-printable.
      key >= 224))) {
        return;
      }
      ...
      if (!faded) {
        ...
        $overlay // Always recalculate the editor area when entering the overlay with the mouse.
        ...
        .on('mousemove.focus', function (event) {
          ...
          if (x && y && (nx !== x || ny !== y)) {
            if (ny <= y && ny < editorRect.top - pageYOffset || ny >= y && ny > editorRect.bottom - pageYOffset || nx <= x && nx < editorRect.left - pageXOffset || nx >= x && nx > editorRect.right - pageXOffset) {
              traveledX += Math.abs(x - nx);
              traveledY += Math.abs(y - ny);

              if ((ny <= editorRect.top - buffer - pageYOffset || ny >= editorRect.bottom + buffer - pageYOffset || nx <= editorRect.left - buffer - pageXOffset || nx >= editorRect.right + buffer - pageXOffset) && (traveledX > 10 || traveledY > 10)) {
                fadeIn();
                x = y = null;
                traveledX = traveledY = 0;
                return;
    ...
    function fadeOutAdminBar() {
      if (!fadedAdminBar && faded) {
        fadedAdminBar = true;
        $adminBar.on('mouseenter.focus', function () {
    ...
    $document.on('tinymce-editor-setup.focus', function (event, editor) {
      editor.addButton('dfw', {
        ...
        onPostRender: function onPostRender() {
          ...
          $document.on('dfw-activate.focus', function () {
    ...
    $document.on('tinymce-editor-init.focus', function (event, editor) {
      ...
      if (editor.id === 'content') {
        ...
        $document.on('dfw-on.focus', mceBind).on('dfw-off.focus', mceUnbind); // Focuse the editor when it is the target of the click event.
    ...
    $document.on('quicktags-init', function (event, editor) {
      var $button; // Bind the distraction free writing events if the distraction free writing button is available.

      if (editor.settings.buttons && (',' + editor.settings.buttons + ',').indexOf(',dfw,') !== -1) {
        $button = $('#' + editor.name + '_dfw');
        $(document).on('dfw-activate', function () {
