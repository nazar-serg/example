...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = function ($, _, wp, api) {
    ...
    /** @lends wp.customize.navMenusPreview.NavMenuInstancePartial.prototype */
    {
      ...
      isRelatedSetting: function isRelatedSetting(setting, newValue, oldValue) {
        ...
        if (isNavMenuItemSetting && _.isObject(newValue) && _.isObject(oldValue)) {
          ...
          delete _oldValue.type_label; // Normalize URL scheme when parent frame is HTTPS to prevent selective refresh upon initial page load.
        ...
        return 'nav_menu[' + navMenuId + ']' === setting.id || isNavMenuItemSetting && (newValue && newValue.nav_menu_term_id === navMenuId || oldValue && oldValue.nav_menu_term_id === navMenuId);
      },
      ...
      refresh: function refresh() {
        var partial = this,
            menuId,
            deferred = $.Deferred(); // Make sure the fallback behavior is invoked when the partial is no longer associated with a menu.
      ...
      renderContent: function renderContent(placement) {
        var partial = this,
            previousContainer = placement.container; // Do fallback behavior to refresh preview if menu is now empty.
  ...
  self.highlightControls = function () {
    var selector = '.menu-item'; // Skip adding highlights if not in the customizer preview iframe.
