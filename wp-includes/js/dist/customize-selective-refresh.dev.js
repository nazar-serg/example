...
wp.customize.selectiveRefresh = function ($, api) {
  ...
  /** @lends wp.customize.SelectiveRefresh.Partial.prototype */
  {
    ...
    initialize: function initialize(id, options) {
      ...
      partial.params = _.extend({
        settings: []
      }, partial.defaults, options.params || options);
      partial.deferred = {};
     ...
     * @return {void}
     */
    addEditShortcutToPlacement: function addEditShortcutToPlacement(placement, $editShortcut) {
     ...
     */
    preparePlacement: function preparePlacement(placement) {
    ...
    renderContent: function renderContent(placement) {
      ...
      try {
        ...
        if (partial.params.containerInclusive) {
          ...
          newContainerElement = $(content); // Merge the new context on top of the old context.

          placement.context = _.extend(placement.context, newContainerElement.data('customize-partial-placement-context') || {});
      ...
      partial.createEditShortcutForPlacement(placement);
      placement.container.removeClass('customize-partial-refreshing'); // Prevent placement container from being re-triggered as being rendered among nested partials.
  ...
  self.partial = new api.Values({
    defaultConstructor: Partial
  });
  /**
  ...
  self.requestPartial = function (partial) {
    ...
    self._debouncedTimeoutId = setTimeout(function () {
      ...
      request.done(function (data) {
         ...
         */
        self.trigger('render-partials-response', data); // Relay errors (warnings) captured during rendering and relay to console.
  ...
  api.bind('preview-ready', function () {
    ...
    _.extend(self.data, _customizePartialRefreshExports); // Create the partial JS models.
    ...
    _.each(self.data.partials, function (data, id) {
      ...
      if (!partial) {
        ...
        partial = new Constructor(id, _.extend({
          params: data
        }, data) // Inclusion of params alias is for back-compat for custom partials that expect to augment this property.
        );
        self.partial.add(partial);
      } else {
    ...
    handleSettingChange = function handleSettingChange(newValue, oldValue) {
      var setting = this;
    ...
    watchSettingChange = function watchSettingChange(setting) {
    ...
    unwatchSettingChange = function unwatchSettingChange(setting) {
