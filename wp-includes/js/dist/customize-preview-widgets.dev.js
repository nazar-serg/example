...
wp.customize.widgetsPreview = wp.customize.WidgetCustomizerPreview = function ($, _, wp, api) {
  ...
  /** @lends wp.customize.widgetsPreview.SidebarPartial.prototype */
  {
    ...
    ready: function ready() {
      var sidebarPartial = this; // Watch for changes to the sidebar_widgets setting.
      ...
      api.selectiveRefresh.bind('partial-content-rendered', function (placement) {
        var isAssignedWidgetPartial = placement.partial.extended(self.WidgetPartial) && -1 !== _.indexOf(sidebarPartial.getWidgetIds(), placement.partial.widgetId);
    ...
    findDynamicSidebarBoundaryNodes: function findDynamicSidebarBoundaryNodes() {
      var partial = this,
          regExp,
          boundaryNodes = {},
          _recursiveCommentTraversal;

      regExp = /^(dynamic_sidebar_before|dynamic_sidebar_after):(.+):(\d+)$/;

      _recursiveCommentTraversal = function recursiveCommentTraversal(childNodes) {
        _.each(childNodes, function (node) {
          ...
          if (8 === node.nodeType) {
            ...
            _recursiveCommentTraversal(node.childNodes);
          }
        });
      };

      _recursiveCommentTraversal(document.body.childNodes);
    ...
    reflowWidgets: function reflowWidgets() {
      ...
      _.each(sidebarPlacements, function (sidebarPlacement) {
        var sidebarWidgets = [],
            needsSort = false,
            thisPosition,
            lastPosition = -1; // Gather list of widget partial containers in this sidebar, and determine if a sort is needed.
        ...
        if (needsSort) {
          _.each(sidebarWidgets, function (sidebarWidget) {
            sidebarPlacement.endNode.parentNode.insertBefore(sidebarWidget.placement.container[0], sidebarPlacement.endNode); // @todo Rename partial-placement-moved?
     ...
     */
    ensureWidgetPlacementContainers: function ensureWidgetPlacementContainers(widgetId) {
      var sidebarPartial = this,
          widgetPartial,
          wasInserted = false,
      ...
      _.each(sidebarPartial.placements(), function (sidebarPlacement) {
        ...
        widgetContainerElement = $(sidebarPartial.params.sidebarArgs.before_widget.replace(/%1\$s/g, widgetId).replace(/%2\$s/g, 'widget') + sidebarPartial.params.sidebarArgs.after_widget); // Handle rare case where before_widget and after_widget are empty.
     ...
     * @return {void}
     */
    handleSettingChange: function handleSettingChange(newWidgetIds, oldWidgetIds) {
      var sidebarPartial = this,
          needsRefresh,
          widgetsRemoved,
          widgetsAdded,
          addedWidgetPartials = [];
      needsRefresh = oldWidgetIds.length > 0 && 0 === newWidgetIds.length || newWidgetIds.length > 0 && 0 === oldWidgetIds.length;
      ...
      _.each(widgetsRemoved, function (removedWidgetId) {
        ...
        if (widgetPartial) {
          _.each(widgetPartial.placements(), function (placement) {
            var isRemoved = placement.context.sidebar_id === sidebarPartial.sidebarId || placement.context.sidebar_args && placement.context.sidebar_args.id === sidebarPartial.sidebarId;
  ...
  self.buildWidgetSelectors = function () {
    ...
    $.each(self.registeredSidebars, function (i, sidebar) {
      var widgetTpl = [sidebar.before_widget, sidebar.before_title, sidebar.after_title, sidebar.after_widget].join(''),
          emptyWidget,
          widgetSelector,
          widgetClasses;
      emptyWidget = $(widgetTpl);
      widgetSelector = emptyWidget.prop('tagName') || '';
      widgetClasses = emptyWidget.prop('className') || ''; // Prevent a rare case when before_widget, before_title, after_title and after_widget is empty.
  ...
  self.highlightControls = function () {
    var self = this,
        selector = this.widgetSelectors.join(','); // Skip adding highlights if not in the customizer preview iframe.

    if (!api.settings.channel) {
      return;
    }

    $(selector).attr('title', this.l10n.widgetTooltip); // Highlights widget when entering the widget editor.
