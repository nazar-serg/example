...
(function ($) {
  ...
  _.extend(wp.Backbone.Subviews.prototype, {
     ...
     */
    set: function set(selector, views, options) {
      var existing, next;
      ...
      if (existing) {
        if (options.add) {
          ...
          _.each(existing, function (view) {
            if (view.__detach) view.$el.detach();else view.remove();
          });
      ...
      if (!options.silent) this._attach(selector, views, _.extend({
        ready: this._isReady()
      }, options));
      return this;
    },
     ...
     * @return {wp.Backbone.Subviews} The current subviews instance.
     */
    add: function add(selector, views, options) {
      if (!_.isString(selector)) {
        options = views;
        views = selector;
        selector = '';
      ...
      return this.set(selector, views, _.extend({
        add: true
      }, options));
    },
     ...
     * @return {wp.Backbone.Subviews} The current Subviews instance.
     */
    unset: function unset(selector, views, options) {
      var existing;
    ...
    */
    remove: function remove(options) {
      if (!options || !options.silent) {
        if (this.parent && this.parent.views) this.parent.views.unset(this.selector, this.view, {
          silent: true
        });
        delete this.parent;
     ...
     * @return {wp.Backbone.Subviews} The current Subviews instance.
     */
    replace: function replace($target, els) {
      $target.html(els);
      return this;
    },
     ...
     * @return {wp.Backbone.Subviews} The current Subviews instance.
     */
    insert: function insert($target, els, options) {
      var at = options && options.at,
          $children;
      if (_.isNumber(at) && ($children = $target.children()).length > at) $children.eq(at).before(els);else $target.append(els);
      return this;
    },
    ...
    ready: function ready() {
      this.view.trigger('ready'); // Find all attached subviews, and call ready on them.
     ...
     * @return {wp.Backbone.Subviews} The current Subviews instance.
     */
    _attach: function _attach(selector, views, options) {
      var $selector = selector ? this.view.$(selector) : this.view.$el,
          managers; // Check if we found a location to attach the views.

      if (!$selector.length) return this;
      managers = _.chain(views).pluck('views').flatten().value(); // Render the views if necessary.

      _.each(managers, function (manager) {
        if (manager.rendered) return;
        manager.view.render();
        manager.rendered = true;
      }, this); // Insert or replace the views.
  ...
  wp.Backbone.View = Backbone.View.extend({
    ...
    remove: function remove() {
      var result = Backbone.View.prototype.remove.apply(this, arguments); // Recursively remove child views.
