...
(function (exports, $) {
  ...
  (function (history) {
    ...
    injectUrlWithState = function injectUrlWithState(url) {
  ...
  /** @lends wp.customize.Preview.prototype */
  {
    ...
    handleLinkClick: function handleLinkClick(event) {
      var preview = this,
          link,
          isInternalJumpLink;
      link = $(event.target).closest('a'); // No-op if the anchor is not a link.
    ...
    handleFormSubmit: function handleFormSubmit(event) {
      ...
      urlParser = document.createElement('a');
      form = $(event.target);
      urlParser.href = form.prop('action'); // If the link is not previewable, prevent the browser from navigating to it.
  ...
  api.addLinkPreviewing = function addLinkPreviewing() {
    var linkSelectors = 'a[href], area[href]'; // Inject links into initial document.
  ...
  api.prepareLinkPreview = function prepareLinkPreview(element) {
    var queryParams,
        $element = $(element); // Skip elements with no href attribute. Check first to avoid more expensive checks down the road.
  ...
  api.addRequestPreviewing = function addRequestPreviewing() {
     ...
     * @return {void}
     */
    var prefilterAjax = function prefilterAjax(options, originalOptions, xhr) {
      var urlParser,
          queryParams,
          requestMethod,
          dirtyValues = {};
      urlParser = document.createElement('a');
      urlParser.href = options.url; // Abort if the request is not for this site.
      ...
      queryParams = api.utils.parseQueryString(urlParser.search.substring(1)); // Note that _dirty flag will be cleared with changeset updates.
      ...
      if (!_.isEmpty(dirtyValues)) {
        requestMethod = options.type.toUpperCase(); // Override underlying request method to ensure unsaved changes to changeset can be included (force Backbone.emulateHTTP).
  ...
  api.prepareFormPreview = function prepareFormPreview(form) {
    ...
    urlParser = document.createElement('a');
    urlParser.href = form.action; // Make sure forms in preview use HTTPS if parent frame uses HTTPS.
  ...
  api.keepAliveCurrentUrl = function () {
    ...
    return function keepAliveCurrentUrl() {
      var urlParser, currentQueryParams; // Short-circuit with keep-alive if previous URL is identical (as is normal case).
  ...
  api.settingPreviewHandlers = {
     ...
     * @return {void}
     */
    custom_logo: function custom_logo(attachmentId) {
     ...
     * @return {void}
     */
    custom_css: function custom_css(value) {
  ...
  $(function () {
    ...
    setValue = function setValue(id, value, createDirty) {
      var setting = api(id);
    ...
    api.preview.bind('active', function () {
      ...
      api.preview.send('documentTitle', document.title); // Send scroll in case of loading via non-refresh.
    ...
    handleUpdatedChangesetUuid = function handleUpdatedChangesetUuid(uuid) {
      api.settings.changeset.uuid = uuid; // Update UUIDs in links and forms.
    ...
    setInterval(api.keepAliveCurrentUrl, api.settings.timeouts.keepAliveSend); // Display a loading indicator when preview is reloading, and remove on failure.
