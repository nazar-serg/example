...
(function ($, _) {
  ...
  wp.media.string = {
     ...
     */
    props: function props(_props, attachment) {
      var link,
          linkUrl,
          size,
          sizes,
          defaultProps = wp.media.view.settings.defaultProps;
      _props = _props ? _.clone(_props) : {};

      if (attachment && attachment.type) {
        _props.type = attachment.type;
      }

      if ('image' === _props.type) {
        _props = _.defaults(_props || {}, {
      ...
      _props.title = _props.title || attachment.title;
      link = _props.link || defaultProps.link || getUserSetting('urlbutton', 'file');
      ...
      _props.linkUrl = linkUrl || ''; // Format properties for images.

      if ('image' === attachment.type) {
        _props.classes.push('wp-image-' + attachment.id);

        sizes = attachment.sizes;
        size = sizes && sizes[_props.size] ? sizes[_props.size] : attachment;

        _.extend(_props, _.pick(attachment, 'align', 'caption', 'alt'), {
          width: size.width,
          height: size.height,
          src: size.url,
        ...
        _.extend(_props, _.pick(attachment, 'title', 'type', 'icon', 'mime')); // Format properties for non-images.

      } else {
        _props.title = _props.title || attachment.filename;
        _props.rel = _props.rel || 'attachment wp-att-' + attachment.id;
      }
     ...
     */
    link: function link(props, attachment) {
      var options;
     ...
     */
    audio: function audio(props, attachment) {
     ...
     */
    video: function video(props, attachment) {
     ...
     */
    _audioVideo: function _audioVideo(type, props, attachment) {
     ...
     * @return {string}
     */
    image: function image(props, attachment) {
      var img = {},
          options,
          classes,
          shortcode,
      ...
      _.extend(img, _.pick(props, 'width', 'height', 'alt')); // Only assign the align class to the image if we're not printing
      ...
      img['class'] = _.compact(classes).join(' '); // Generate `img` tag options.
      ...
      html = wp.html.string(options); // Generate the caption shortcode.
  ...
  wp.media.collection = function (attributes) {
    ...
    /** @lends wp.media.collection.prototype */
    {
       ...
       */
      attachments: function attachments(shortcode) {
        ...
        args.type = this.type;
        args.perPage = -1; // Mark the `orderby` override attribute.
       ...
       * @return {wp.shortcode}
       */
      shortcode: function shortcode(attachments) {
        ...
        attrs.ids = attachments.pluck('id'); // Copy the `uploadedTo` post ID.
        ...
        delete attrs._orderByField; // If the `ids` attribute is set and `orderby` attribute
      ...
      edit: function edit(content) {
        var shortcode = wp.shortcode.next(this.tag, content),
            ...
            attachments,
            selection,
            state; // Bail if we didn't match the shortcode or all of the content.
        ...
        selection[this.tag] = attachments[this.tag]; // Fetch the query's attachments, and then break ties from the
      ...
      setDefaults: function setDefaults(attrs) {
        var self = this; // Remove default attributes from the shortcode.
  ...
  wp.media.featuredImage = {
    ...
    init: function init() {
      $('#postimagediv').on('click', '#set-post-thumbnail', function (event) {
        event.preventDefault(); // Stop propagation to prevent thickbox from activating.
  ...
  wp.media.editor = {
    ...
    add: function add(id, options) {
      var workflow = this.get(id); // Only add once: if exists return existing.
    ...
    id: function id(_id) {
      ...
      _id = window.wpActiveEditor; // If that doesn't work, fall back to `tinymce.activeEditor.id`.

      if (!_id && !_.isUndefined(window.tinymce) && tinymce.activeEditor) {
        _id = tinymce.activeEditor.id;
    ...
    send: {
       ...
       * @return {Promise}
       */
      attachment: function attachment(props, _attachment) {
        var caption = _attachment.caption,
            options,
            html; // If captions are disabled, clear the caption.

        if (!wp.media.view.settings.captions) {
          delete _attachment.caption;
        }

        props = wp.media.string.props(props, _attachment);
        options = {
          id: _attachment.id,
          post_content: _attachment.description,
        ...
        } else if ('video' === _attachment.type) {
          html = wp.media.string.video(props, _attachment);
        } else if ('audio' === _attachment.type) {
          html = wp.media.string.audio(props, _attachment);
        } else {
     ...
     */
    open: function open(id, options) {
      var workflow;
      options = options || {};
      id = this.id(id);
      this.activeEditor = id;
      workflow = this.get(id); // Redo workflow if state has changed.
