...
window.wpCookies = {
  ...
  each: function each(obj, cb, scope) {
    var n, l;
   ...
   */
  setHash: function setHash(name, values_obj, expires, path, domain, secure) {
    var str = '';
  ...
  set: function set(name, value, expires, path, domain, secure) {
    ...
    if (_typeof(expires) === 'object' && expires.toGMTString) {
...
window.setUserSetting = function (name, value, _del) {
  if ('object' !== (typeof userSettings === "undefined" ? "undefined" : _typeof(userSettings))) {
    return false;
  }
