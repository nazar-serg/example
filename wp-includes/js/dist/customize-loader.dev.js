...
(function (exports, $) {
  ...
  /** @lends wp.customize.Loader.prototype */
  {
    ...
    initialize: function initialize() {
      this.body = $(document.body); // Ensure the loader is supported.
      ...
      this.window = $(window);
      this.element = $('<div id="customize-container" />').appendTo(this.body); // Bind events for opening and closing the overlay.

      this.bind('open', this.overlay.show);
      this.bind('close', this.overlay.hide); // Any element in the body with the `load-customize` class opens
      // the Customizer.

      $('#wpbody').on('click', '.load-customize', function (event) {
        event.preventDefault(); // Store a reference to the link that opened the Customizer.

        Loader.link = $(this); // Load the theme.
    ...
    open: function open(src) {
      ...
      this.iframe.one('load', this.loaded); // Create a postMessage connection with the iframe.
      ...
      if (history.replaceState) {
        this.messenger.bind('changeset-uuid', function (changesetUuid) {
          ...
          urlParser.search = $.param(_.extend(api.utils.parseQueryString(urlParser.search.substr(1)), {
    ...
    pushState: function pushState(src) {
      var hash = src.split('?')[1]; // Ensure we don't call pushState if the user hit the forward button.
    ...
    close: function close() {
      ...
      _onConfirmClose = function onConfirmClose(confirmed) {
        if (confirmed) {
          self.active = false;
          self.trigger('close'); // Restore document title prior to opening the Live Preview.
        ...
        self.messenger.unbind('confirmed-close', _onConfirmClose);
      };

      self.messenger.bind('confirmed-close', _onConfirmClose);
