...
(function (window, o, undef) {
  var delay = window.setTimeout,
      fileFilters = {}; // convert plupload features to caps acceptable by mOxie
  ...
  var plupload = {
    ...
    */
    ...
    cleanName: function cleanName(name) {
      var i, lookup; // Replace diacritics
      ...
      name = name.replace(/\s+/g, '_'); // Remove anything else
    ...
    formatSize: function formatSize(size) {
      ...
      var boundary = Math.pow(1024, 4); // TB
  ...
  plupload.addFileFilter('max_file_size', function (maxSize, file, cb) {
    var undef;
    maxSize = plupload.parseSize(maxSize); // Invalid file size
  ...
  */
  ...
  plupload.Uploader = function (options) {
    ...
    function calc() {
      var i, file; // Reset stats

      total.reset(); // Check status, size, loaded etc on all files
      ...
      for (i = 0; i < files.length; i++) {
        ...
        if (file.size !== undef) {
          ...
          total.size += file.origSize; // Since we cannot predict file size after resize, we do opposite and
        ...
        plupload.each(settings.browse_button, function (el) {
          queue.push(function (cb) {
            ...
            fileInput.onready = function () {
              var info = o.Runtime.getInfo(this.ruid); // for backward compatibility
    ...
    function _setOption2(option, value, init) {
      ...
      if (init) {
        ...
        settings.required_features = normalizeCaps(plupload.extend({}, settings)); // Come up with the list of capabilities that can affect default mode in a multi-mode runtimes
    ...
    function onUploadFile(up, file) {
      var url = up.settings.url,
          ...
          offset = 0,
          blob; // make sure we start at a predictable offset
      ...
      function uploadNextChunk() {
        var chunkBlob,
            formData,
            args = {},
            curChunkSize; // make sure that file wasn't cancelled and upload is not stopped in general
        ...
        xhr = new o.XMLHttpRequest(); // Do we have upload progress support
        ...
        if (up.settings.multipart && features.multipart) {
          xhr.open("post", url, true); // Set custom headers

          plupload.each(up.settings.headers, function (value, name) {
            xhr.setRequestHeader(name, value);
          });
          formData = new o.FormData(); // Add multipart params
      ...
      blob = file.getSource(); // Start uploading chunks
    ...
    function onFileUploaded(up) {
      calc(); // Upload next file but detach it from the error event
    ...
    function onError(up, err) {
      if (err.code === plupload.INIT_ERROR) {
        up.destroy();
          ...
          calcFile(err.file); // Upload next file but detach it from the error event
          // since other custom listeners might want to stop the queue
    ...
    _setOption2.call(this, options, null, true); // Inital total state


    total = new plupload.QueueProgress(); // Add public methods
    ...
    plupload.extend(this, {
      ...
      init: function init() {
        ...
        bindEventListeners.call(self); // Check for required options
                ...
                queue.push(function (cb) {
                  ...
                  filterFile(file, function (err) {
                    if (!err) {
                      ...
                      files.push(file); // collect the files that will be passed to FilesAdded event
      ...
      splice: function splice(start, length) {
        ...
        var removed = files.splice(start === undef ? 0 : start, length === undef ? files.length : length); // if upload is in progress we need to stop it and restart after files are removed
        ...
        this.trigger("FilesRemoved", removed); // Dispose any resources allocated by those files
