...
(function ($) {
  ...
  $.scheduler.prototype = {
    schedule: function schedule() {
      ...
      if (_typeof(arguments[i]) == "object" && arguments.length > 1) {
        override = true;
        i++;
      }
      ...
      if (_typeof(arguments[i]) == "object" && typeof arguments[i + 1] == "string" && _isfn(arguments[i][arguments[i + 1]])) {
        ctx["obj"] = arguments[i++];
      ...
      if (!_isfn(ctx["func"])) {
        if (ctx["obj"] != null && _typeof(ctx["obj"]) == "object" && typeof ctx["func"] == "string" && _isfn(ctx["obj"][ctx["func"]])) ctx["func"] = ctx["obj"][ctx["func"]];else ctx["func"] = eval("function () { " + ctx["func"] + " }");
      }
    ...
    reschedule: function reschedule(ctx) {
      if (typeof ctx == "string") ctx = this.bucket[ctx];
      ctx["_handle"] = this._schedule(ctx);
      return ctx;
    },
    _schedule: function _schedule(ctx) {
      var trampoline = function trampoline() {
        var obj = ctx["obj"] != null ? ctx["obj"] : ctx;
        ctx["func"].apply(obj, ctx["args"]);
        if (typeof ctx["_scheduler"].bucket[ctx["id"]] != "undefined" && ctx["repeat"]) ctx["_scheduler"]._schedule(ctx);else delete ctx["_scheduler"].bucket[ctx["id"]];
      };
