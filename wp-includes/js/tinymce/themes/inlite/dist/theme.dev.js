...
(function () {
  var inlite = function (domGlobals) {
    ...
    var result = function result(id, rect) {
      return {
        id: id,
        rect: rect
      };
    ...
    var match = function match(editor, matchers) {
    ...
    var toAbsolute = function toAbsolute(rect) {
    ...
    var measureElement = function measureElement(elm) {
    ...
    var getElementRect = function getElementRect(editor, elm) {
      return measureElement(elm);
    };

    var getPageAreaRect = function getPageAreaRect(editor) {
      return measureElement(editor.getElement().ownerDocument.body);
    };

    var getContentAreaRect = function getContentAreaRect(editor) {
      return measureElement(editor.getContentAreaContainer() || editor.getBody());
    };

    var getSelectionRect = function getSelectionRect(editor) {
    ...
    var element = function element(_element, predicateIds) {
      return function (editor) {
        for (var i = 0; i < predicateIds.length; i++) {
          if (predicateIds[i].predicate(_element)) {
            var result = Matcher.result(predicateIds[i].id, Measure.getElementRect(editor, _element));
            return result;
          }
        }
    ...
    var parent = function parent(elements, predicateIds) {
      return function (editor) {
    ...
    var create = function create(id, predicate) {
      return {
        id: id,
        predicate: predicate
      };
    ...
    var fromContextToolbars = function fromContextToolbars(toolbars) {
    ...
    var textSelection = function textSelection(id) {
      return function (editor) {
    ...
    var emptyTextBlock = function emptyTextBlock(elements, id) {
      return function (editor) {
        var i;
    ...
    var isObject = function isObject(predicate) {
      return function (value) {
    ...
    var getByTypeOr = function getByTypeOr(predicate) {
    ...
    var splitNoEmpty = function splitNoEmpty(str, delim) {
      return str.split(delim).filter(function (item) {
        return item.length > 0;
      });
    };

    var itemsToArray = function itemsToArray(value, defaultValue) {
      var stringToItemsArray = function stringToItemsArray(value) {
        return typeof value === 'string' ? splitNoEmpty(value, /[ ,]/) : value;
      };

      var boolToItemsArray = function boolToItemsArray(value, defaultValue) {
    ...
    var getToolbarItemsOr = function getToolbarItemsOr(predicate) {
      return function (editor, name, defaultValue) {
    ...
    var result$1 = function result$1(rect, position) {
      return {
        rect: rect,
        position: position
      };
    ...
    var moveTo = function moveTo(rect, toRect) {
      return {
        x: toRect.x,
        y: toRect.y,
        w: rect.w,
    ...
    var calcByPositions = function calcByPositions(testPositions1, testPositions2, targetRect, contentAreaRect, panelRect) {
    ...
    var calcInsert = function calcInsert(targetRect, contentAreaRect, panelRect) {
      return calcByPositions(['cr-cl', 'cl-cr'], ['bc-tc', 'bl-tl', 'br-tr'], targetRect, contentAreaRect, panelRect);
    };

    var calc = function calc(targetRect, contentAreaRect, panelRect) {
      return calcByPositions(['tc-bc', 'bc-tc', 'tl-bl', 'bl-tl', 'tr-br', 'br-tr', 'cr-cl', 'cl-cr'], ['bc-tc', 'bl-tl', 'br-tr', 'cr-cl'], targetRect, contentAreaRect, panelRect);
    };

    var userConstrain = function userConstrain(handler, targetRect, contentAreaRect, panelRect) {
    ...
    var defaultHandler = function defaultHandler(rects) {
    ...
    var toAbsoluteUrl = function toAbsoluteUrl(editor, url) {
      return editor.documentBaseURI.toAbsolute(url);
    };

    var urlFromName = function urlFromName(name) {
    ...
    var getPositionHandler = function getPositionHandler(editor) {
    ...
    var fireSkinLoaded$1 = function fireSkinLoaded$1(editor, callback) {
    ...
    var load = function load(editor, callback) {
    ...
    var getSelectionElements = function getSelectionElements(editor) {
      var node = editor.selection.getNode();
      var elms = editor.dom.getParents(node, '*');
      return elms;
    };

    var createToolbar = function createToolbar(editor, selector, id, items) {
      var selectorPredicate = function selectorPredicate(elm) {
    ...
    var getToolbars = function getToolbars(editor) {
      ...
      return DeepFlatten.flatten([contextToolbars ? contextToolbars : [], createToolbar(editor, 'img', 'image', 'alignleft aligncenter alignright')]);
    };

    var findMatchResult = function findMatchResult(editor, toolbars) {
      ...
      result = Matcher.match(editor, [ElementMatcher.element(elements[0], contextToolbarsPredicateIds), SelectionMatcher.textSelection('text'), SelectionMatcher.emptyTextBlock(elements, 'insert'), ElementMatcher.parent(elements, contextToolbarsPredicateIds)]);
      return result && result.rect ? result : null;
    };

    var editorHasFocus = function editorHasFocus(editor) {
      return domGlobals.document.activeElement === editor.getBody();
    };

    var togglePanel = function togglePanel(editor, panel) {
    ...
    var repositionPanel = function repositionPanel(editor, panel) {
      return function () {
        var toolbars = getToolbars(editor);
        var result = findMatchResult(editor, toolbars);
    ...
    var ignoreWhenFormIsVisible = function ignoreWhenFormIsVisible(editor, panel, f) {
      return function () {
    ...
    var bindContextualToolbarsEvents = function bindContextualToolbarsEvents(editor, panel) {
    ...
    var overrideLinkShortcut = function overrideLinkShortcut(editor, panel) {
    ...
    var renderInlineUI = function renderInlineUI(editor, panel) {
    ...
    var renderUI = function renderUI(editor, panel) {
    ...
    proto = {
      ...
      slice: function slice() {
        return new Collection$1(_slice.apply(this, arguments));
      },
    ...
    function DragHelper(id, settings) {
      ...
      start = function start(e) {
        ...
        global$7(doc).on('mousemove touchmove', drag).on('mouseup touchend', _stop);
        settings.start(e);
      };
      ...
      _stop = function stop(e) {
        updateWithTouchData(e);
        global$7(doc).off('mousemove touchmove', drag).off('mouseup touchend', _stop);
        $eventOverlay.remove();
    ...
    function toggleBodyFullScreenClasses(classPrefix, state) {
      if (checkFullscreenWindows() && state === false) {
        global$7([domGlobals.document.documentElement, domGlobals.document.body]).removeClass(classPrefix + 'fullscreen');
      }
    }
    ...
    var get = function get(editor, panel) {
    ...
    var create$1 = function create$1(dom, rng) {
      var bookmark = {};
    ...
    var resolve$1 = function resolve$1(dom, bookmark) {
    ...
    var formatBlock = function formatBlock(editor, formatName) {
    ...
    var changeHref = function changeHref(editor, elm, url) {
      editor.focus();
    ...
    var addHeaderButtons = function addHeaderButtons(editor) {
      var formatBlock = function formatBlock(name) {
        return function () {
    ...
    var addToEditor = function addToEditor(editor, panel) {
    ...
    var getUiContainerDelta$1 = function getUiContainerDelta$1() {
    ...
    var focusFirstTextBox = function focusFirstTextBox(form) {
      form.find('textbox').eq(0).each(function (ctrl) {
        ctrl.focus();
      });
    };

    var createForm = function createForm(name, spec) {
    ...
    var toggleVisibility = function toggleVisibility(ctrl, state) {
      return state ? ctrl.show() : ctrl.hide();
    };

    var askAboutPrefix = function askAboutPrefix(editor, href) {
      return new global$c(function (resolve) {
    ...
    var convertLinkToAbsolute = function convertLinkToAbsolute(editor, href) {
      return !UrlType.isAbsolute(href) && UrlType.isDomainLike(href) ? askAboutPrefix(editor, href) : global$c.resolve(href);
    };

    var createQuickLinkForm = function createQuickLinkForm(editor, hide) {
      var attachState = {};
      ...
      var onShowHandler = function onShowHandler(e) {
        if (e.control === this) {
          var elm = void 0,
              linkurl = '';
    ...
    var getSelectorStateResult = function getSelectorStateResult(itemName, item) {
      var result = function result(selector, handler) {
        return {
          selector: selector,
          handler: handler
        };
      ...
      var activeHandler = function activeHandler(state) {
        item.active(state);
      };

      var disabledHandler = function disabledHandler(state) {
        item.disabled(state);
      };
    ...
    var bindSelectorChanged = function bindSelectorChanged(editor, itemName, item) {
      return function () {
    ...
    var itemsToArray$1 = function itemsToArray$1(items) {
    ...
    var create$2 = function create$2(editor, name, items) {
      var toolbarItems = [];
      var buttonGroup;
    ...
    var create$3 = function create$3() {
      var panel, currentRect;

      var createToolbars = function createToolbars(editor, toolbars) {
        return global$4.map(toolbars, function (toolbar) {
          return Toolbar.create(editor, toolbar.id, toolbar.items);
        });
      };

      var hasToolbarItems = function hasToolbarItems(toolbar) {
        return toolbar.items().length > 0;
      };

      var create = function create(editor, toolbars) {
        var items = createToolbars(editor, toolbars).concat([Toolbar.create(editor, 'text', Settings.getTextSelectionToolbarItems(editor)), Toolbar.create(editor, 'insert', Settings.getInsertToolbarItems(editor)), Forms.createQuickLinkForm(editor, hide)]);
        return global$b.create({
          type: 'floatpanel',
          role: 'dialog',
      ...
      var showPanel = function showPanel(panel) {
        if (panel) {
          panel.show();
        }
      };
      ...
      var togglePositionClass = function togglePositionClass(panel, relPos) {
      ...
      var showToolbar = function showToolbar(panel, id) {
      ...
      var repositionPanelAt = function repositionPanelAt(panel, id, editor, targetRect) {
      ...
      var showPanelAt = function showPanelAt(panel, id, editor, targetRect) {
        showPanel(panel);
        panel.items().hide();
      ...
      var hasFormVisible = function hasFormVisible() {
        return panel.items().filter('form:visible').length > 0;
      };

      var showForm = function showForm(editor, id) {
        if (panel) {
          panel.items().hide();
      ...
      var show = function show(editor, id, targetRect, toolbars) {
        if (!panel) {
      ...
      var reposition = function reposition(editor, id, targetRect) {
        if (panel) {
    ...
    var create$4 = function create$4(type, title, url, level, attach) {
      return {
        type: type,
        title: title,
        url: url,
    ...
    var TextBox = Widget.extend({
      ...
      renderHtml: function renderHtml() {
        ...
        global$4.each(['rows', 'spellcheck', 'maxLength', 'size', 'readonly', 'min', 'max', 'step', 'list', 'pattern', 'placeholder', 'required', 'multiple'], function (name) {
