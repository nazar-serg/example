...
var tinyMCEPopup = {
  ...
  init: function init() {
    ...
    self.dom.bind(window, 'ready', self._onDOMLoaded, self); // Enables you to skip loading the default css
   ...
   */
  getWindowArg: function getWindowArg(name, defaultValue) {
   ...
   */
  getLang: function getLang(name, defaultValue) {
   ...
   */
  execCommand: function execCommand(cmd, ui, val, args) {
    args = args || {};
    args.skip_focus = 1;
   ...
   */
  executeOnLoad: function executeOnLoad(evil) {
   ...
   */
  pickColor: function pickColor(e, element_id) {
    var el = document.getElementById(element_id),
        colorPickerCallback = this.editor.settings.color_picker_callback;

    if (colorPickerCallback) {
      colorPickerCallback.call(this.editor, function (value) {
        el.value = value;

        try {
          el.onchange();
        } catch (ex) {// Try fire event, ignore errors
        }
      }, el.value);
    }
  },
   ...
   */
  openBrowser: function openBrowser(element_id, type) {
   ...
   */
  confirm: function confirm(t, cb, s) {
  ...
  close: function close() {
    var t = this; // To avoid domain relaxing issue in Opera
  ...
  _onDOMLoaded: function _onDOMLoaded() {
    ...
    if (t.features.translate_i18n !== false) {
      ...
      h = document.body.innerHTML; // Replace a=x with a="x" in IE
    ...
    document.body.style.display = ''; // Restore selection in IE when focus is placed on a non textarea or input element of the type text

    if (tinymce.Env.ie) {
      if (tinymce.Env.ie < 11) {
        document.attachEvent('onmouseup', tinyMCEPopup._restoreSelection); // Add base target element for it since it would fail with modal dialogs
    ...
    t.restoreSelection();
    t.resizeToInnerSize(); // Set inline title
    ...
    if (t.getWindowArg('mce_auto_focus', true)) {
      window.focus(); // Focus element with mceFocus class
  ...
  _accessHandler: function _accessHandler(e) {
    e = e || window.event;
  ...
  _closeWinKeyHandler: function _closeWinKeyHandler(e) {
    e = e || window.event;
  ...
  _eventProxy: function _eventProxy(id) {
    return function (evt) {
...
tinymce.util.Dispatcher = function (scope) {
  ...
  this.dispatch = function () {
    var self = this,
        ...
        listener;
    self.inDispatch = true; // Needs to be a real loop since the listener count might change while looping
