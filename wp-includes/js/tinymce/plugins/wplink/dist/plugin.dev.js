  ...
  tinymce.PluginManager.add('wplink', function (editor) {
    ...
    editor.on('preinit', function () {
      if (editor.wp && editor.wp._createToolbar) {
        toolbar = editor.wp._createToolbar(['wp_link_preview', 'wp_link_edit', 'wp_link_remove'], true);
    ...
    editor.addShortcut('access+s', '', 'wp_unlink'); // The "de-facto standard" shortcut, see #27305.
    ...
    editor.on('BeforeAddUndo', function (event) {
      if (event.lastLevel && event.lastLevel.content && event.level.content && event.lastLevel.content === removePlaceholderStrings(event.level.content)) {
        event.preventDefault();
      }
    ...
    editor.on('keydown', function (event) {
      ...
      if (event.altKey || tinymce.Env.mac && (!event.metaKey || event.ctrlKey) || !tinymce.Env.mac && !event.ctrlKey) {
        return;
      }
      ...
      onPostRender: function onPostRender() {
        ...
        if ($ && $.ui && $.ui.autocomplete) {
          ...
          $input.on('keydown', function () {
            ...
            return $('<li role="option" id="mce-wp-autocomplete-' + item.ID + '">').append('<span>' + title + '</span>&nbsp;<span class="wp-editor-float-right">' + item.info + '</span>').appendTo(ul);
          };
          ...
          .autocomplete('widget').addClass('wplink-autocomplete').attr('role', 'listbox').removeAttr('tabindex') // Remove the `tabindex=0` attribute added by jQuery UI.
