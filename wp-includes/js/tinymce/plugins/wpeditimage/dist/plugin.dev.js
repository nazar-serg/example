...
tinymce.PluginManager.add('wpeditimage', function (editor) {
  ...
  editor.addButton('wp_img_edit', {
    tooltip: 'Edit|button',
    // '|button' is not displayed, only used for context.
  ...
  editor.once('preinit', function () {
    if (editor.wp && editor.wp._createToolbar) {
      toolbar = editor.wp._createToolbar(['wp_img_alignleft', 'wp_img_aligncenter', 'wp_img_alignright', 'wp_img_alignnone', 'wp_img_edit', 'wp_img_remove']);
    }
  });
  ...
  function extractImageData(imageNode) {
    var classes,
        ...
        captionClassName = [],
        dom = editor.dom,
        isIntRegExp = /^\d+$/; // Default attributes.
    ...
    metadata.extraClasses = extraClasses.join(' '); // Extract caption.
  ...
  function verifyHTML(caption) {
    ...
    return serializer.serialize(editor.parser.parse(caption, {
  ...
  function updateImage($imageNode, imageData) {
    ...
    dom.setAttribs(imageNode, attrs); // Preserve empty alt attributes.
    ...
    if (imageData.caption) {
      ...
      if (captionNode) {
        ...
        id = id ? 'id="' + id + '" ' : ''; // Should create a new function for generating the caption markup.

        html = '<dl ' + id + 'class="' + className + '" style="width: ' + width + 'px">' + '<dt class="wp-caption-dt"></dt><dd class="wp-caption-dd">' + imageData.caption + '</dd></dl>';
        wrap = dom.create('div', {
          'class': 'mceTemp'
        }, html);
    ...
    src = $imageNode.attr('src'); // Remove srcset and sizes if the image file was edited or the image was replaced.
  ...
  function editImage(img) {
    ...
    metadata = extractImageData(img); // Mark the image node so we can select it later.

    editor.$(img).attr('data-wp-editing', 1); // Manipulate the metadata by reference that is fed into the PostImage model used in the media modal.
    ...
    callback = function callback(imageData) {
  ...
  editor.on('init', function () {
    ...
    dom.addClass(editor.getBody(), captionClass); // Prevent IE11 from making dl.wp-caption resizable.
  ...
  editor.on('pastePostProcess', function (event) {
    ...
    if (editor.dom.getParent(editor.selection.getNode(), 'dd.wp-caption-dd')) {
      ...
      editor.$('br', event.node).each(function (i, node) {
        if (!node.nextSibling || node.nextSibling.nodeName === 'BR' || !node.previousSibling || node.previousSibling.nodeName === 'BR') {
          editor.dom.remove(node);
        }
  ...
  (function () {
    ...
    editor.on('drop', function (event) {
      var dom = editor.dom,
          rng = tinymce.dom.RangeUtils.getCaretRangeFromPoint(event.clientX, event.clientY, editor.getDoc()); // Don't allow anything to be dropped in a captioned image.
  ...
  editor.wp = editor.wp || {};
  editor.wp.isPlaceholder = isPlaceholder; // Back-compat.
