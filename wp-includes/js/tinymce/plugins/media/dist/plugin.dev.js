...
(function () {
  var media = function () {
    ...
    var trimPx$1 = function trimPx$1(value) {
      return value.replace(/px$/, '');
    };

    var addPx$1 = function addPx$1(value) {
      return /^[0-9.]+$/.test(value) ? value + 'px' : value;
    };
    ...
    var doSyncSize = function doSyncSize(widthCtrl, heightCtrl) {
      widthCtrl.state.set('oldVal', widthCtrl.value());
      heightCtrl.state.set('oldVal', heightCtrl.value());
    };

    var doSizeControls = function doSizeControls(win, f) {
    ...
    var doUpdateSize = function doUpdateSize(widthCtrl, heightCtrl, isContrained) {
    ...
    var syncSize = function syncSize(win) {
      doSizeControls(win, doSyncSize);
    };

    var updateSize = function updateSize(win) {
      doSizeControls(win, doUpdateSize);
    };

    var createUi = function createUi(onChange) {
      var recalcSize = function recalcSize() {
    ...
    var getData = function getData(editor) {
      ...
      return element.getAttribute('data-mce-object') ? HtmlToData.htmlToData(Settings.getScripts(editor), editor.serializer.serialize(element, {
        selection: true
      })) : {};
    };

    var getSource = function getSource(editor) {
    ...
    var addEmbedHtml = function addEmbedHtml(win, editor) {
    ...
    var submitForm = function submitForm(win, editor) {
      var data = win.toJSON();
    ...
    var populateMeta = function populateMeta(win, meta) {
    ...
    var showDialog = function showDialog(editor) {
      ...
      var reserialise = function reserialise(update) {
        update(win);
        data = win.toJSON();
      ...
      var updateValueOnChange = function updateValueOnChange() {
    ...
    var isWithinEphoxEmbed = function isWithinEphoxEmbed(node) {
