...
var mergeConfigToEnvFlags = require('./lib/shared/config/env-flags'); // Logging functions


var logVerify = require('./lib/shared/log/verify');

var logBlacklistError = require('./lib/shared/log/blacklist-error'); // Get supported ranges


var ranges = fs.readdirSync(path.join(__dirname, '/lib/versioned/')); // Set env var for ORIGINAL cwd
...
function run() {
  cli.prepare({
    ...
    env.configProps = cfg; // Set up event listeners for logging again after configuring.
...
module.exports = run; // The actual logic
...
function handleArguments(env) {
  ...
  if (!env.modulePath) {
    ...
    var missingNodeModules = fs.existsSync(path.join(env.cwd, 'package.json')) && !fs.existsSync(path.join(env.cwd, 'node_modules'));
    /* istanbul ignore next */

    var missingGulpMessage = missingNodeModules ? 'Local modules not found in' : 'Local gulp not found in';
    log.error(ansi.red(missingGulpMessage), ansi.magenta(tildify(env.cwd)));
    var hasYarn = fs.existsSync(path.join(env.cwd, 'yarn.lock'));
    /* istanbul ignore next */

    var installCommand = missingNodeModules ? hasYarn ? 'yarn install' : 'npm install' : hasYarn ? 'yarn add gulp' : 'npm install gulp';
  ...
  if (process.cwd() !== env.cwd) {
    ...
    log.info('Working directory changed to', ansi.magenta(tildify(env.cwd)));
  ...
  if (!range) {
    log.error(ansi.red('Unsupported gulp version', env.modulePackage.version));
    exit(1);
