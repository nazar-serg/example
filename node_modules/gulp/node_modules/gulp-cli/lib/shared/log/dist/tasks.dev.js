...
function getLineInfoCollector(lineInfos) {
  return {
    ...
    childTask: function childTask(node) {
      lineInfos.push({
        name: node.label,
        type: 'child'
      });
...
function getNodeFactory(getTask, entryObserver) {
  return {
    topNode: function topNode(node) {
      return {
        label: node.label
      };
    },
    taskNode: function taskNode(node) {
    ...
    childNode: function childNode(node) {
      var newChild = {
        label: node.label
      };
