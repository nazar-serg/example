...
var initialize = function initialize(inputCss, options) {
  var testCase = {};
  ...
  testCase.result = false; // Probe filesystem once and cache the results
...
var runTest = function runTest(inputCssPath, options) {
...
var executeSuite = function executeSuite(suite, tests) {
  ...
  tests = tests.filter(function (test) {
    ...
    var inputSass = testSuiteFolder[suiteFolderLength + 1]; // Add the test if the specPath matches the testname
  ...
  if (tests.length !== 0) {
    ...
    for (var i = 0; i < tests.length; i++) {
      ...
      if (suiteName !== prevSuite) {
        suite.suites.push(executeSuite({
          name: prevSuite,
...
var runSuites = function runSuites(suite) {
