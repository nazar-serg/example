...
function appears(entry) {
  console.error(indent + 'a %s appears!', entry.type, entry.basename, _typeof(entry.basename));
  ...
  if (foggy) {
    ...
    throw new Error("\x1B[mshould not have entries while foggy");
  }
...
function missile(entry) {
  function liftFog(who) {
    ...
    console.error("\x1B[mthe fog lifts!\n");
    clearTimeout(foggy);
    foggy = null;
...
w.on('entry', attacks); // w.on('ready', function () { attacks(w) })
...
r.on('end', function () {
  ...
  console.error("\x1B[mIT'S OVER!!");
  console.error('A WINNAR IS YOU!');
  console.log('ok ' + i++ + ' A WINNAR IS YOU');
  ended = true; // now go through and verify that everything in there is a dir.

  var p = path.resolve(__dirname, 'filter-copy');
  var checker = fstream.Reader({
    path: p
  });
