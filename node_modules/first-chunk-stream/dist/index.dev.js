...
function FirstChunkStream(options, cb) {
  ...
  Duplex.call(this, options); // Initialize the internal state

  _state.manager = createReadStreamBackpressureManager(this); // Errors management
  ...
  this.on('error', _state.errorHandler); // Callback management
...
util.inherits(FirstChunkStream, Duplex); // Utils to manage readable stream backpressure
...
function createReadStreamBackpressureManager(readableStream) {
  var manager = {
    ...
    programPush: function programPush(chunk, encoding, done) {
      done = done || function () {}; // Store the current write


      manager.programmedPushs.push([chunk, encoding, done]); // Need to be async to avoid nested push attempts
      // Programm a push attempt

      setImmediate(manager.attemptPush); // Let's say we're ready for a read
  ...
  readableStream._read = function streamFilterRestoreRead() {
    manager.waitPush = true; // Need to be async to avoid nested push attempts
