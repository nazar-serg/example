...
function processExecSync(file, args, options) {
  ...
  */
  ...
  options = options || {}; // init timeout

  timeout = Date.now() + options.timeout; // init tmpdir
  ...
  fs.mkdirSync(tmpdir); // init command

  if (os === 'linux') {
    command = '(' + command + ' > ' + tmpdir + '/stdout 2> ' + tmpdir + '/stderr); echo $? > ' + tmpdir + '/status';
  } else {
    command = '(' + command + ' > ' + tmpdir + '/stdout 2> ' + tmpdir + '/stderr) | echo %errorlevel% > ' + tmpdir + '/status | exit';
  } // init child
