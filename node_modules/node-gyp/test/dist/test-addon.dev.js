...
test('build simple addon', function (t) {
  t.plan(3); // Set the loglevel otherwise the output disappears when run via 'npm test'
...
test('build simple addon in path with non-ascii characters', function (t) {
  ...
  var testDirName = testDirNames[getEncoding()]; // If encoding is UTF-8 or other then no need to test
  ...
  var testNodeDir = path.join(addonPath, testDirName); // Create symbol link to path with non-ascii characters
  ...
  var cmd = [nodeGyp, 'rebuild', '-C', addonPath, '--loglevel=verbose', '-nodedir=' + testNodeDir];
  var proc = execFile(process.execPath, cmd, function (err, stdout, stderr) {
    try {
